...
void merge(vector<T>& arr, int const left, int const mid, int const right) {
  ...
  for (int i = 0; i < subArrayOneSize; i++) leftArray[i] = arr[left + i];
  for (int j = 0; j < subArrayTwoSize; j++) rightArray[j] = arr[mid + 1 + j];
...
int main() {
  cout << "You can now go checkout the output files in the same folder !";
